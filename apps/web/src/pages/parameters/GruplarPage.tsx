...
import { FolderTree, Search, Plus, X, Pencil, Trash2 } from 'lucide-react'
...
export function GruplarPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/product-groups?${params}`)
  ...
  const openEdit = (item: ProductGroup) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const url = editingId ? `${API_URL}/api/product-groups/${editingId}` : `${API_URL}/api/product-groups`
  ...
  return (
    <PageLayout
      title="Gruplar"
      description="Ürün gruplarını yönetin"
      backTo="/parametreler"
      showRefresh
      onRefresh={() => {
        setSearch('')
      ...
      headerActions={
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" title="Yeni grup" onClick={openNew}>
